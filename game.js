const LESSONS = {
  ...
  kalimat: {
    ...
    header: "Kastil Kalimat",
    mission: "MISI: KALIMAT MINI",
...
const targetCard = el("targetCard");
const targetHint = el("targetHint");
const feedback = el("feedbackMessage");
const startBtn = el("startBtn");
const restartBtn = el("restartBtn");
const scoreStat = el("scoreStat");
const comboStat = el("comboStat");
const starStat = el("starStat");
const roundLabel = el("roundLabel");
const progressPercent = el("progressPercent");
const progressBar = el("progressBar");
const chestMilestone = el("chestMilestone");
const rewardPop = el("rewardPop");
...
const chestOverlay = el("chestOverlay");
const openChestBtn = el("openChestBtn");
const continueChestBtn = el("continueChestBtn");
const chestIconUse = el("chestIconUse");
const chestReward = el("chestReward");
const chestRewardTitle = el("chestRewardTitle");
const chestRewardXp = el("chestRewardXp");
const finishOverlay = el("finishOverlay");
const finishScore = el("finishScore");
const finishStars = el("finishStars");
const finishXp = el("finishXp");
const finishMessage = el("finishMessage");
const finishLevelText = el("finishLevelText");
const finishXpBar = el("finishXpBar");
const badgeToast = el("badgeToast");
const badgeToastName = el("badgeToastName");
const soundToggle = el("soundToggle");
const soundIconUse = el("soundIconUse");
const kiboMini = el("kiboMini");
const kiboSpeech = el("kiboGameSpeech");
const sessionXpEl = el("sessionXp");
const miniXpBar = el("miniXpBar");
const levelText = el("levelText");
const headerLevel = el("headerLevel");
...
function storageSet(key, value) {
  try { localStorage.setItem(key, value); } catch { /* game tetap berjalan */ }
}
...
function levelInfo(xp = adventure.xp) {
  ...
  return {
    ...
    current: safe % XP_PER_LEVEL,
    percent: ((safe % XP_PER_LEVEL) / XP_PER_LEVEL) * 100
  };
}
...
function setIcon(useEl, iconId) {
  if (useEl) useEl.setAttribute("href", `${ICON_SPRITE}#${iconId}`);
}
...
function currentTarget() {
  return String(queue[currentRound] ?? lesson.items[0]);
}
...
function renderHud() {
  const done = Math.min(currentRound, lesson.rounds);
  const percent = Math.round((done / lesson.rounds) * 100);
...
function unlockBadge(id) {
  ...
  saveAdventure();
  badgeToastName.textContent = BADGES[id].name;
...
function awardXp(amount) {
  ...
  renderLevel();
  if (after > before) {
    setKibo(`LEVEL UP! Sekarang kamu Level ${after}!`, "happy");
    createConfetti(28);
    speak(`Level naik. Sekarang level ${after}`);
  }
}
...
function createConfetti(amount = 18) {
  ...
  for (let i = 0; i < amount; i += 1) {
    ...
    piece.style.top = `${15 + Math.random() * 20}%`;
...
function resetGame() {
  ...
  setIcon(chestIconUse, "chest-closed");
...
function runCountdown() {
  ...
  const step = () => {
    ...
    countdownNumber.textContent = current.number;
    countdownText.textContent = current.text;
    countdownNumber.style.animation = "none";
    void countdownNumber.offsetWidth;
    countdownNumber.style.animation = "countPop .65s ease";
    speak(current.number === "GO!" ? "Mulai" : current.number);
    index += 1;
    if (index < steps.length) countdownTimer = setTimeout(step, 700);
    else countdownTimer = setTimeout(beginRound, 600);
  };
  step();
}
...
function wrongAnswer() {
  ...
  setFeedback("Hampir! Lihat kartunya lagi lalu coba sekali lagi.", "warning");
...
function correctAnswer() {
  ...
  const earnedScore = 100 + Math.min(combo * 10, 100);
  ...
  if (combo >= 3) speak(`Hebat. Combo ${combo}`);

  advanceTimer = setTimeout(() => {
    const isLast = currentRound + 1 >= lesson.rounds;
    if (correct % 5 === 0) {
      chestFinishesGame = isLast;
      openChestCheckpoint();
      return;
    }
    if (isLast) {
...
function evaluate() {
  ...
  const typedLower = typed.toLocaleLowerCase("id-ID");
  const targetLower = target.toLocaleLowerCase("id-ID");
  if (targetLower.startsWith(typedLower)) {
    targetCard.classList.remove("wrong");
    setFeedback(lesson.type === "number" ? "Bagus, lanjutkan angkanya." : "Bagus, lanjutkan sedikit lagi.");
  } else {
    wrongAnswer();
  }
}
...
function revealChest() {
  ...
  saveAdventure();
  renderHud();
  setIcon(chestIconUse, "chest-open");
  chestRewardTitle.textContent = `+${bonusStars} bintang`;
  chestRewardXp.textContent = `+${bonusXp} XP`;
  chestReward.hidden = false;
  continueChestBtn.hidden = false;
  openChestBtn.hidden = true;
  createConfetti(30);
  setKibo("Hore! Hadiahnya sudah kita ambil.", "happy");
  speak(`Hore. Kamu mendapat ${bonusStars} bintang dan ${bonusXp} XP.`);
}
...
function finishGame() {
  ...
  if (!adventure.completed.includes(currentLesson)) {
  ...
  const info = levelInfo();
  finishLevelText.textContent = `Level ${info.level} • ${info.current}/${XP_PER_LEVEL} XP`;
  finishXpBar.style.width = `${info.percent}%`;
  ...
  const bestKey = `ketik100-best-${currentLesson}`;
  const best = Number(storageGet(bestKey) || 0);
  if (score > best) storageSet(bestKey, String(score));

  setKibo("Dunia selesai! Aku bangga sama usahamu.", "happy");
  createConfetti(42);
  finishOverlay.hidden = false;
  speak("Petualangan selesai. Hebat sekali!");
}

el("headerWorld").textContent = lesson.header;
el("worldLabel").textContent = lesson.world;
el("practiceTitle").textContent = lesson.title;
el("missionPill").textContent = lesson.mission;
el("instruction").textContent = lesson.instruction;
...
el("speakTarget").addEventListener("click", speakTarget);
...
soundToggle.addEventListener("click", () => {
  ...
  else speak("Suara panduan aktif");
});
